use oxwc::{state::{init_wayland_listener, Oxwc}, Result, CompositorError};
use smithay::{
    backend::{
        renderer::{
...
fn main() -> Result<()> {
    ...
    let mut event_loop: EventLoop<Oxwc> = EventLoop::try_new()
        .map_err(|e| CompositorError::EventLoop(e.to_string()))?;

    let display: Display<Oxwc> = Display::new()
    ...
    let (mut winit_backend, mut winit_event_loop) = winit::init::<GlowRenderer>()
    ...
    let winit_output = state.add_output(
        "winit".to_string(),
        window_size.w,
        window_size.h,
    );
        ...
        .insert_source(
            ...
            move |_, _, state| {
                ...
                for window in state.space.elements() {
                    let window_location = state.space.element_geometry(window)
                        .map(|geo| geo.loc)
                        .unwrap_or_default();

                    let elements: Vec<_> = window.render_elements::<WaylandSurfaceRenderElement<GlowRenderer>>(
                        renderer,
                ...
                let mut frame = renderer.render(&mut target, size, smithay::utils::Transform::Flipped180)
                ...
                for element in &render_elements {
                    ...
                    element.draw(&mut frame, src, dst, &[damage], &[damage]).ok();
                }

                let _ = frame.finish();

                drop(target);
                winit_backend.submit(Some(&[damage])).expect("submit failed");
